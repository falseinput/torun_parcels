...
| Node | Only for `make glyphs`, not for the pipeline | 18 |
...
| `make glyphs` | Regenerates `site/glyphs/` (outside `make all` -- see below) |
| `make clean` | Deletes `build/` and `dist/` |

## Parcel number labels

At z17 and above each parcel is labelled with its number. No label point is
stored in the tiles: MapLibre anchors a polygon symbol at the polygon's pole of
inaccessibility, the point furthest from any edge, which stays inside the
L-shaped and crescent parcels where a centroid would fall outside.

Rendering text needs a `glyphs` endpoint, and the hosted ones require an API
key, so the font is self-hosted. `site/glyphs/Noto Sans Regular/0-255.pbf` is a
checked-in build *input*, generated by `make glyphs` from Noto Sans and pinned
to a Noto release tag. Parcel numbers are digits and `/`, so it only needs
regenerating if the font or the label alphabet changes -- which is why it sits
outside `make all`, along with its Node dependency.
